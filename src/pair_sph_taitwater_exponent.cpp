...
void PairSPHTaitwaterExponent::compute(int eflag, int vflag) {
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // fi = tmp * tmp * tmp;
    // double old_exp_res = (fi * fi * tmp - 1.0);
    // double new_exp_res = pow(tmp, exponent[itype]) - 1.0;
    // if (exponent[itype] == 1.0) printf("tmp: %f exponent[itype]: %f old_exp_res: %f new_exp_res: %f\n", tmp, exponent[itype], old_exp_res, new_exp_res);
    // fi = B[itype] * (fi * fi * tmp - 1.0) / (rho[i] * rho[i]);
    fi = B[itype] * (pow(tmp, exponent[itype]) - 1.0) / (rho[i] * rho[i]);
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        fj = B[jtype] * (pow(tmp, exponent[jtype]) - 1.0) / (rho[j] * rho[j]);
...
void PairSPHTaitwaterExponent::allocate() {
  ...
  memory->create(exponent, n + 1, "pair:exponent");
...
void PairSPHTaitwaterExponent::coeff(int narg, char **arg) {
  ...
  double exponent_one = force->numeric(FLERR,arg[6]);
  double B_one = soundspeed_one * soundspeed_one * rho0_one / 7.0;

  // printf("exponent_one: %d\n", exponent_one);

  // printf("rho0_one: %f soundspeed_one: %f viscosity_one: %f cut_one: %f exponent_one: %f\n", 
  // 	rho0_one, soundspeed_one, viscosity_one, cut_one, exponent_one);

  int count = 0;
  for (int i = ilo; i <= ihi; i++) {
    rho0[i] = rho0_one;
    soundspeed[i] = soundspeed_one;
    exponent[i] = exponent_one;
